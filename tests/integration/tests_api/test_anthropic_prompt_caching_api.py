...
class TestAnthropicPromptCachingAPI:
    ...
    def test_sync_completion(self, sync_client):
        ...
        garbage_response = simple_anthropic_client.messages.create(
        ...
        try:
            ...
            assert (
                usage_db.prompt_cached_creation_tokens
        ...
        assert usage_last.providers[0].total_tokens == response.usage.input_tokens + response.usage.output_tokens
        assert (
            usage_last.providers[0].prompt_tokens_details.cached_input_tokens
            == getattr(response.usage, 'cache_read_input_tokens', 0)
        )
        assert (
            usage_last.providers[0].prompt_tokens_details.cached_creation_tokens
            == getattr(response.usage, 'cache_creation_input_tokens', 0)
        )
