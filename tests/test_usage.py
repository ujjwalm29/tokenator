...
def test_last_hour(mock_session, mock_usage_data, base_time):
    ...
    expected_data = [
        record for record in mock_usage_data 
...
def test_between_date_filtering(mock_session, mock_usage_data, base_time):
    ...
    for case in test_cases:
        ...
        assert result.total_tokens == case["expected_tokens"], f"Failed for format: start={type(case['start'])}, end={type(case['end'])}"
...
def test_between_provider_filtering(mock_session, mock_usage_data, base_time):
    ...
    expected_data = [
        record for record in mock_usage_data 
        if (datetime.strptime(start_date, "%Y-%m-%d") <= record.created_at <= 
...
def test_between_model_filtering(mock_session, mock_usage_data, base_time):
    ...
    assert len(result.providers) == 1
    assert all(model.model == "gpt-4" for provider in result.providers for model in provider.models)
...
def test_last_execution(mock_session, mock_usage_data):
    ...
    expected_data = [u for u in mock_usage_data if u.execution_id == most_recent.execution_id]
    mock_session.query().filter().all.return_value = expected_data
